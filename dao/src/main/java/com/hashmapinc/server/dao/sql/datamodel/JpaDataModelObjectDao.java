...
public class JpaDataModelObjectDao extends JpaAbstractSearchTextDao<DataModelObjectEntity, DataModelObject> implements DataModelObjectDao {
    ...
    public List<DataModelObject> findByDataModelIdAndType(DataModelId dataModelId, String type) {
        List<DataModelObjectEntity> entities = dataModelObjectRespository.findByDataModelIdAndType(UUIDConverter.fromTimeUUID(dataModelId.getId()), type);
        return DaoUtil.convertDataList(entities);
    }
    ...
    public List<DataModelObject> findByName(String name) {
        List<DataModelObjectEntity> entities = dataModelObjectRespository.findByName(name);
        return DaoUtil.convertDataList(entities);
    }
}

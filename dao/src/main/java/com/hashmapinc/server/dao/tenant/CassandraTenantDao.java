...
import com.hashmapinc.server.dao.model.nosql.TenantEntity;
...
public class CassandraTenantDao extends CassandraAbstractSearchTextDao<TenantEntity, Tenant> implements TenantDao {
    ...
    public List<Tenant> findTenantsByRegion(String region, TextPageLink pageLink) {
        ...
        List<TenantEntity> tenantEntities = findPageWithTextSearch(ModelConstants.TENANT_BY_REGION_AND_SEARCH_TEXT_COLUMN_FAMILY_NAME,
                Arrays.asList(eq(ModelConstants.TENANT_REGION_PROPERTY, region)),
                pageLink); 

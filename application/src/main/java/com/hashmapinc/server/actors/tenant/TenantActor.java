...
public class TenantActor extends ContextAwareActor {
    ...
    private void onComponentLifecycleMsg(ComponentLifecycleMsg msg) {
        if (msg.getPluginId().isPresent()) {
            ...
            msg.getDashboardId().ifPresent(dashboardId -> onComponentLifecycleMsgForDashboard(msg, dashboardId));
        ...
        if(msg.getEvent().equals(ComponentLifecycleEvent.DELETED)) {
            ...
            if(msg.getComputationJobId().isPresent()) {
                msg.getComputationJobId().ifPresent(computationJobId -> {
                    ComputationJobDeleteMessage computationJobDeleteMessage = new ComputationJobDeleteMessage(computationJobId);
    ...
    private void onComponentLifecycleMsgForDashboard(ComponentLifecycleMsg msg, DashboardId dashboardId) {
        if(msg.getEvent().equals(ComponentLifecycleEvent.DELETED)) {
            ActorRef applicationActor = applicationManager.getOrCreateApplicationActor(this.context());
            DashboardDeleteMessage dashboardDeleteMessage = new DashboardDeleteMessage(dashboardId);
    ...
    private void onComponentLifecycleMsgForRule(ComponentLifecycleMsg msg, RuleId ruleId) {
        if(msg.getEvent().equals(ComponentLifecycleEvent.DELETED)) {
            ActorRef applicationActor = applicationManager.getOrCreateApplicationActor(this.context());
            RuleDeleteMessage ruleDeleteMessage = new RuleDeleteMessage(ruleId);

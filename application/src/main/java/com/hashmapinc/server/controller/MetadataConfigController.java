...
import com.hashmapinc.server.common.data.id.ApplicationId;
...
public class MetadataConfigController extends BaseController {
    ...
    public void insert(MetadataIngestionEntries metadataIngestionEntries) throws TempusException {
        ...
        Optional<Entitlements> entitlements = entitlementsService.findEntitlementsForUserId(securityUser.getId());
        if(entitlements.isPresent() && entitlements.get().getEntitledServices().contains(EntitledServices.METADATA_INGESTION)) {

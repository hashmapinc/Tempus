...
export function DataModelController($scope, $mdDialog, $document, $stateParams, $timeout, $q, datamodelService, toast, $translate) {
    ...
    function onDatamodelObjectSelect(properties) {
        ...
        if (vm.childNode.length > 0) {
            vm.childName = vm.childNode.join(',')
        }
    ...
    vm.showDatamodelObjectStepper = function(targetEvent, nodeToEdit) {
        ...
        if (nodeToEdit) {
            ...
            if (child_nodes.length > 0) {
                vm.stepperData.child_node_id = child_nodes;
            }
    ...
    function getChildNodes(node_id, child_node) {


        var old_edges = vm.edges.get().filter(edge => {
            return edge.from === node_id;
        });

        if (old_edges.length > 0) {
            for (var i = 0; i < old_edges.length; i++) {
                child_node.push(old_edges[i].to);
                getChildNodes(old_edges[i].to, child_node);
            }
    ...
    vm.onStepperDelete = function() {
        ...
        if (angular.isDefined(vm.stepperData.child_node_id)) {
            childNodes = vm.getNodes();

            angular.forEach(childNodes, function(value, key) {

                if (child_nodes.indexOf(value.id) != -1) {

                    vm.childArray.push(value.label);
                }
        ...
        $mdDialog.show(confirm).then(function() {
            ...
            if (angular.isDefined(vm.stepperData.child_node_id)) {
                angular.forEach(childNodes, function(value, key) {
                    if (child_nodes.indexOf(value.id) != -1) {
                        vm.nodes.remove(value.id);
                        objectDeleteList.push(value.datamodelObject.id);
                        objectDeleteNameList.push(value.datamodelObject.name);
                    }
                });
